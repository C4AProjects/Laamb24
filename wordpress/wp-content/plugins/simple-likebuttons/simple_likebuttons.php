...
//add_filter('the_title', 'add_simple_likebuttons_totitle');
...
//Add icons method
function add_simple_likebuttons_totitle($strTitle) {

  //Only show at a single post / page the extended (if configurated) view
  if(is_single() or is_page()) {
    return $strTitle . getLikeButtonsHtmlCode('small');
  } else {
    return $strTitle . getLikeButtonsHtmlCode('small');
  }
...
function get_simplelikes(){
	 return getLikeButtonsHtmlCode('small');
}
add_shortcode("simplelikes", "get_simplelikes");
